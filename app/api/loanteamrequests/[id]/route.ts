...
export async function GET(request: NextRequest, {params} : {params: {id: string}}) {
    ...
    const outgoingLoanTeamRequests: LoanTeamRequest[] = await Promise.all(requestorTeamRequests.map(async (request) => {
        ...
        return {...request, requesteeEmail: requestee?.email || null};
    }))
    ...
    const incomingLoanTeamRequests: LoanTeamRequest[] = await Promise.all(requesteeTeamRequests.map(async (request) => {
        const requestor = await prisma.user.findUnique({
            where: {id: request.requestorId}
        })
        return {...request, requestorEmail: requestor?.email || null};
    }))
